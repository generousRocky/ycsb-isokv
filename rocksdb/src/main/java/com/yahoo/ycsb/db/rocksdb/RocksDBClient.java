...
public class RocksDBClient extends DB {
  ...
  private RocksDB initRocksDB() throws IOException, RocksDBException {
    ...
    if(cfDescriptors.isEmpty()) {
      System.out.println("[rocky_dbg][java] open path 1");
      ...
      options.setEnv(options.getEnv());
      options.getEnv().setBackgroundThreads(1);
      
      dbOptions = options;
      return RocksDB.open(options, rocksDbDir.toAbsolutePath().toString());
    } else {
      System.out.println("[rocky_dbg][java] open path 2");
